...
def get_metadata(folder):
    ...
    with open(metadata / 'mappings' / 'folder_to_objectnet_label.json', 'r') as f:
        folder_map = json.load(f)
        folder_map = {v: k for k, v in folder_map.items()}
    with open(metadata / 'mappings' / 'objectnet_to_imagenet_1k.json', 'r') as f:
        objectnet_map = json.load(f)

    with open(metadata / 'mappings' / 'pytorch_to_imagenet_2012_id.json', 'r') as f:
        pytorch_map = json.load(f)
        pytorch_map = {v: k for k, v in pytorch_map.items()}

    with open(metadata / 'mappings' / 'imagenet_to_label_2012_v2', 'r') as f:
...
class ObjectNetDataset(datasets.ImageFolder):
    ...
    def __init__(self, root, transform):
        ...
        subdir = os.path.join(root, 'images')
