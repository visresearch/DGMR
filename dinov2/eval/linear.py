...
def get_args_parser(
    ...
    parser.add_argument(
        "--fnames-ckpt-classifiers",
        nargs="+",
        type=str,
        help="Path to classifier ckpt",
    )
...
def setup_linear_classifiers_only_test(sample_output, n_last_blocks_list, num_classes=1000):
    linear_classifiers_dict = nn.ModuleDict()
    ...
    for n in n_last_blocks_list:
        ...
        linear_classifier = linear_classifier.cuda()
        linear_classifiers_dict[
            f"classifier_{n}_blocks_avgpool_{avgpool}"
        ] = linear_classifier
...
def run_eval_linear_only_test(
    backbone,
    fnames_ckpt_classifiers,
    output_dir,
    val_dataset_str,
    batch_size,
    num_workers,
    ...
    num_classes = len(torch.unique(torch.Tensor(val_dataset.get_targets().astype(int))))

    n_last_blocks_list = [1, 4]
    n_last_blocks = max(n_last_blocks_list)
    autocast_ctx = partial(torch.cuda.amp.autocast, enabled=True, dtype=autocast_dtype)
    feature_model = ModelWithIntermediateLayers(backbone, n_last_blocks, autocast_ctx)
    sample_output = feature_model(val_dataset[0][0].unsqueeze(0).cuda())

    linear_classifiers = setup_linear_classifiers_only_test(
        sample_output,
        n_last_blocks_list,
        num_classes,
    )

    for i, (k, v) in enumerate(linear_classifiers.classifiers_dict.items()):
        state_dict = torch.load(fnames_ckpt_classifiers[i])
        v.linear.load_state_dict(state_dict, strict=True)
...
def main_only_test(args):
    model, autocast_dtype = setup_and_build_model(args)

    run_eval_linear_only_test(
        backbone=model,
        fnames_ckpt_classifiers=args.fnames_ckpt_classifiers,
        output_dir=args.output_dir,
        val_dataset_str=args.val_dataset_str,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
